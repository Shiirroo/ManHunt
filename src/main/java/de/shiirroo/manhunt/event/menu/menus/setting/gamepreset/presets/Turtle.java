...
public class Turtle extends GamePreset {
    ...
    public boolean setPlayersGroup() {
        ...
        if (Bukkit.getOnlinePlayers().stream().filter(e -> !e.getGameMode().equals(GameMode.SPECTATOR)).count() > 1 && ManHuntPlugin.getPlayerData().getPlayersByRole(ManHuntRole.Speedrunner).size() >= getSpeedRunnerSize()) {
            return true;
        }
        return false;
    }
